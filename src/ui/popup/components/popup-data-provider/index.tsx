...
export const PopupDataProvider = ({ tabId, children }: PopupDataProviderProps) => {
  ...
  const onCollect = useCallback(
    async (username: string, password: string) => {
      ...
      addLog('Starting data collectionÃ¢â‚¬Â¦');
      ...
      try {
        ...
        if (!authResponse?.success) {
          const msg = (authResponse as { message?: string })?.message ?? 'Authentication failed.';
          setStatus(PopupStatusType.WARN, msg);
          return;
        }

        addLog('Authentication sent, waiting for routerÃ¢â‚¬Â¦');
        const extractedData = await startRetryLoop(tabId);

        if (!extractedData) {
          setStatus(PopupStatusType.ERR, 'Timeout waiting for router data.');
          return;
        }
        ...
        if (!parsed.success) {
          setStatus(PopupStatusType.WARN, 'Unexpected data format from router.');
          return;
        }
        ...
        if (isExpectedNavigationError(msg)) {
          addLog('Router redirected, retryingÃ¢â‚¬Â¦', PopupStatusType.WARN);
          ...
          setStatus(PopupStatusType.ERR, 'Error communicating with router.');
    ...
    async (ip: string, mode: DiagnosticsMode) => {
      ...
      try {
        ...
        if (!response?.success || !response.pingResult) {
          const msg = (response as { message?: string })?.message ?? 'Ping failed.';
          setStatus(PopupStatusType.WARN, msg);
          return;
        }
        ...
        setStatus(PopupStatusType.OK, 'Ping completed.');
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        setStatus(PopupStatusType.ERR, 'Error communicating with router.');
        addLog(msg, PopupStatusType.ERR);
  ...
  const copyText = useCallback(async (): Promise<{ data: string | null; error?: string }> => {
    ...
    const boolText = (v: boolean | undefined): string =>
      v === undefined ? '-' : v ? 'Enabled' : 'Disabled';
